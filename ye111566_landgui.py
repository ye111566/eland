...
class Ye111566Landgui(Plugin):
    ...
    def on_command(self, sender: CommandSender, command: Command, args: list[str]) -> bool:
        ...
        if command.name == "landgui":
            if len(args) == 1:
                ...
                elif args[0] == "member_online":
                    self.show_member_online_management_form(sender)
                elif args[0] == "buy":
                ...
                elif args[0] == "transfer_online":
                    self.show_transfer_online_form(sender)
                elif args[0] == "arcb":
    ...
    def show_transfer_online_form(self, sender: CommandSender):
        player = self.server.get_player(sender.name)
        ...
        self.transfer_playerlist=playerlist
        # 创建 ModalForm 菜单
        form = ModalForm(
            title="过户领地(在线版)",
            controls=[
                Dropdown(label="选择领地", options=land_options),      # 下拉选择领地
                Dropdown(label="选择过户的玩家", options=playerlist),   # 输入新名字
            ],
            submit_button="过户",
            on_submit=self.handle_transfer_online_submit
        )
    ...
    # 处理过户表单提交
    def handle_transfer_online_submit(self, player: Player, json_str: str) -> None:
        data = json.loads(json_str)

        land_options = [list(land.keys())[0] for land in self.load_land_data(player.name)]  # 再次获取领地名称

        land_name_index = int(data[0])  # 领地选项的索引
        playerlist=self.transfer_playerlist
        new_land_owner = playerlist[int(data[1])]  # 新领地主人
        if " " in new_land_owner:
            new_land_owner=f'"{new_land_owner}"'
        if not new_land_owner:
            player.send_message("过户的对象不能为空！")
    ...
    def show_manage_form(self, player):
        ...
        permission_types = ["containter", "build", "mine", "tp","mobgriefing","fire","explode"]
        ...
        self.server.get_player(player.name).send_form(
            ModalForm(
                ...
                controls=[
                    Dropdown(label="选择领地", options=land_names),
                    Dropdown(label="选择权限", options=['领地内右键方块','领地内放置','领地内挖掘','公开领地传送',"领地内生物破坏","领地内着火","领地内爆炸"]),
...
# 显示成员online管理表单
    def show_member_online_management_form(self, sender: CommandSender):
        player = self.server.get_player(sender.name)
        ...
        land_options = [list(land.keys())[0] for land in land_data[player_name]]  # 玩家拥有的领地名称列表
        playerlist=[oneplayer.name for oneplayer in self.server.online_players]
        self.member_playerlist=playerlist
        # 创建 ModalForm 菜单
        form = ModalForm(
            title="管理领地成员(在线版)",
            controls=[
                Dropdown(label="选择领地", options=land_options),      # 下拉选择领地
                Dropdown(label="操作", options=["add", "del"]),       # 下拉选择操作
                Dropdown(label="选择玩家", options=playerlist),  # 输入玩家名字
            ],
            submit_button="提交",
            on_submit=self.handle_member_online_submit
        )
        ...
        # 处理成员表单提交
    def handle_member_online_submit(self,player: Player, json_str: str) -> None:
        data = json.loads(json_str)
        land_options = [list(land.keys())[0] for land in self.load_land_data(player.name)]  # 再次获取领地名称
        operation_options = ["add", "del"]
        ...
        land_name_index = int(data[0])  # 领地选项的索引
        operation_index = int(data[1])  # 操作选项的索引
        playername = self.member_playerlist[int(data[2])]  # 玩家名字
        if ' ' in playername:
            playername=f'"{playername}"'
        # 根据索引获取相应的值
        land_name = land_options[land_name_index]
