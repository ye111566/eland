...
class Ye111566Land(Plugin):
    ...
    def on_enable(self) -> None:
        ...
        print("领地数据已更新，'anti_right_click_block' 已初始化。")
                    ...
                    for land_name, land_info in land.items():
                        ...
                        if player_dim == land_dim:
                            ...
                            if (min(posa[0], posb[0]) <= player_x <= max(posa[0], posb[0]) and
                                ...
                                player_name = f'"{oneplayer.name}"' if ' ' in oneplayer.name else oneplayer.name
            ...
            # 8. 管理领地权限
                ...
                if len(args) >= 4:
                    ...
                    if permission_type not in valid_permissions:
                        sender.send_message(f"无效的权限类型！有效类型有: {', '.join(valid_permissions)}")
                        return True
